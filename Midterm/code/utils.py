...
def load_path(filepath, train=True):
    image_paths = []
    classes = []
    ...
    if train:
        for data_path in glob.glob(filepath + '/*'):
            ...
            data_class = data_path.split('_')[-1].split('.')[0]
...
class LandmarkDataset(Dataset):
    def __init__(self, image_paths, class_to_idx=None, transform=False):
    ...
    def __getitem__(self, idx):
        ...
        if self.class_to_idx is not None:
            label = image_filepath.split('_')[-1].split('.')[0]
